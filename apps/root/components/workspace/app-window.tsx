...
export function AppWindowRoot({
  ...
  const { state, surfaceRef, requestPlacement, closeProvider, windowSession } =
    useRuntime();
  const onStage = state.provider.placement === "stage";
  const interactive = onStage && state.motion !== "suction";
