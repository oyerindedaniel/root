...
const PROVIDER_TONES: readonly ProviderTone[] = [
  {
    id: "catalog",
    ground: "var(--amber-ink)",
    ink: "var(--amber-solid)",
    rotate: 55,
    scale: 5,
  },
  {
    ...
    rotate: 150,
    scale: 2.5,
    shift: "translate(-40 -12)",
  },
  {
    id: "cases",
    ground: "var(--indigo-ink)",
    ink: "var(--indigo-mute)",
    rotate: 35,
    scale: 4.5,
    shift: "translate(-24 -36)",
  },
];

function ProviderPatternTile({ tone }: { tone: ProviderTone }) {
  const patternId = `sign-in-pattern-${tone.id}`;
  return (
    <svg
    ...
    >
      <defs>
        <pattern
          ...
          width="100"
          height="100"
          patternTransform={`rotate(${tone.rotate}) scale(${tone.scale})`}
          patternUnits="userSpaceOnUse"
        >
          <rect width="100%" height="100%" fill={tone.ground} />
          <path fill={tone.ink} d={PATTERN_PATH} />
        </pattern>
      </defs>
      <rect
        width="200%"
        height="200%"
        fill={`url(#${patternId})`}
        transform={tone.shift}
      />
    </svg>
  );
}

export function SignInProviderPattern() {
  return (
    <div
      className="grid h-28 shrink-0 grid-cols-3 overflow-hidden"
      aria-hidden="true"
    >
      {PROVIDER_TONES.map((tone) => (
        <ProviderPatternTile key={tone.id} tone={tone} />
      ))}
    </div>
  );
}
